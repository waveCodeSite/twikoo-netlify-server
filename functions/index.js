...
router.post('/', cors(), async (req, res) => {
    ...
    console.log('请求 IP：', getIp(req))
    console.log('请求函数：', event.event)
    console.log('请求参数：', event)
    let response = {}
    try {
        protect(req)
        accessToken = anonymousSignIn(req)
        ...
        switch (event.event) {
            case 'GET_FUNC_VERSION':
                response = getFuncVersion({VERSION})
                break
            case 'COMMENT_GET':
                response = await commentGet(event)
                break
            case 'COMMENT_GET_FOR_ADMIN':
                response = await commentGetForAdmin(event)
                break
            case 'COMMENT_SET_FOR_ADMIN':
                res = await commentSetForAdmin(event)
                break
            case 'COMMENT_DELETE_FOR_ADMIN':
                response = await commentDeleteForAdmin(event)
                break
            case 'COMMENT_IMPORT_FOR_ADMIN':
                response = await commentImportForAdmin(event)
                break
            case 'COMMENT_LIKE':
            ...
            case 'POST_SUBMIT':
                ...
                break
            case 'COUNTER_GET':
                response = await counterGet(event)
                break
            case 'GET_PASSWORD_STATUS':
                response = await getPasswordStatus(config, VERSION)
                break
            case 'SET_PASSWORD':
                response = await setPassword(event)
                break
            case 'GET_CONFIG':
                response = await getConfig({config, VERSION, isAdmin: isAdmin()})
                break
            case 'GET_CONFIG_FOR_ADMIN':
                response = await getConfigForAdmin({config, isAdmin: isAdmin()})
                break
            case 'SET_CONFIG':
                response = await setConfig(event)
                break
            case 'LOGIN':
                response = await login(event.password)
                break
            case 'GET_COMMENTS_COUNT': // >= 0.2.7
                response = await getCommentsCount(event)
                break
            case 'GET_RECENT_COMMENTS': // >= 0.2.7
                response = await getRecentComments(event)
                break
            case 'EMAIL_TEST': // >= 1.4.6
                response = await emailTest(event, config, isAdmin())
                break
            case 'UPLOAD_IMAGE': // >= 1.5.0
                response = await uploadImage(event, config)
                break
            case 'COMMENT_EXPORT_FOR_ADMIN': // >= 1.6.13
                response = await commentExportForAdmin(event)
                break
            default:
                if (event.event) {
                    response.code = RES_CODE.EVENT_NOT_EXIST
                    response.message = '请更新 Twikoo 云函数至最新版本'
                } else {
                    response.code = RES_CODE.NO_PARAM
                    response.message = 'Twikoo 云函数运行正常，请参考 https://twikoo.js.org/quick-start.html#%E5%89%8D%E7%AB%AF%E9%83%A8%E7%BD%B2 完成前端的配置'
        ...
        response.code = RES_CODE.FAIL
        response.message = e.message
    }
    if (!response.code && !req.body.accessToken) {
